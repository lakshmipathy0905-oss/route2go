...
Deno.serve(async (req: Request) => {
  ...
  // Places-along-route is a catalog read; cap per-key rate so a single client
  // can't scan the whole places table on a loop.
  ...
  if (placeId) {
    const { data, error } = await supabase
      .from("places")
      .select(
        "id, name, category_id, lat, lng, photos, hours, entry_fee, rating, description",
      )
      .eq("id", placeId)
      .maybeSingle();
    if (error) {
      return jsonError(500, "DB_ERROR", "Could not load place.", reqId, true);
    }
    if (!data) return jsonOk([], reqId);
    const { data: cat, error: catErr } = await supabase.from("place_categories")
      .select("name").eq("id", data.category_id).maybeSingle();
  ...
  const routeDistanceKm = url.searchParams.get("route_distance_km")
    ? Number(url.searchParams.get("route_distance_km"))
    : null;
  const routeDurationMin = url.searchParams.get("route_duration_min")
    ? Number(url.searchParams.get("route_duration_min"))
    : null;
  const fuelCostPerKm = url.searchParams.get("fuel_cost_per_km")
    ? Number(url.searchParams.get("fuel_cost_per_km"))
    : null;
  ...
  let q = supabase.from("places").select(
    "id, name, category_id, lat, lng, photos, hours, entry_fee, rating, description, place_categories(name)",
  );
  if (categoryIds.length > 0) q = q.in("category_id", categoryIds);

  // Push the corridor filter into SQL: every point within `radiusKm` of the
  // origin->dest segment lies inside this padded bounding box, so the box is a
  // strict superset of what the JS filter below can return. This turns a
  // full-table scan into an index-backed range query ((lat,lng) index).
  const padDeg = Math.min(radiusKm / 100, 5);
  q = q
  ...
  if (error) {
    return jsonError(500, "DB_ERROR", "Could not load places.", reqId, true);
  }

  const corridorKm = routeDistanceKm ??
    haversineKm(originLat, originLng, destLat, destLng);

  const scored = (places ?? [])
    .map((p) => {
      const distKm = pointToSegmentKm(
        p.lat,
        p.lng,
        originLat,
        originLng,
      ...
      const detourKm = routeDistanceKm !== null
        ? (distKm + routeDistanceKm * 0.08)
        : distKm;
      const detourDurationMin = routeDurationMin !== null
        ? Math.round(
          (detourKm /
            (corridorKm / Math.max((routeDurationMin ?? 60) / 60, 1))) || 15,
        )
        : Math.round((detourKm / 45) * 60);
      const detourAddedCost = fuelCostPerKm !== null
        ? Math.round(detourKm * fuelCostPerKm * 100) / 100
        : Math.round(detourKm * 6 * 100) / 100;
