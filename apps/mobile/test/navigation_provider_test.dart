...
ProviderContainer _container({
  ...
  final container = ProviderContainer(
    overrides: [
      ...
      navigationRepositoryProvider.overrideWithValue(repo ?? _FakeNavRepository()),
      ...
      connectivityProvider.overrideWith((ref) => connectivity ?? Stream.value(true)),
    ],
  );
...
void main() {
  ...
  FakeLocationSource onRoute() => FakeLocationSource(
        points: [
          LocationUpdate(lat: 0, lng: 0, timestamp: DateTime(2026, 1, 1, 12, 0, 0)),
          LocationUpdate(lat: 0, lng: 0.3, timestamp: DateTime(2026, 1, 1, 12, 0, 1)),
          LocationUpdate(lat: 0, lng: 0.6, timestamp: DateTime(2026, 1, 1, 12, 0, 2)),
          LocationUpdate(lat: 0, lng: 0.8, timestamp: DateTime(2026, 1, 1, 12, 0, 3)),
        ],
  ...
  FakeLocationSource offRoute() => FakeLocationSource(
        points: [
          LocationUpdate(lat: 0, lng: 0, timestamp: DateTime(2026, 1, 1, 12, 0, 0)),
          LocationUpdate(lat: 0, lng: 0.1, timestamp: DateTime(2026, 1, 1, 12, 0, 1)),
          // 0.01deg north is ~1.1km from the route — beyond the 300m threshold.
          LocationUpdate(lat: 0.01, lng: 0.2, timestamp: DateTime(2026, 1, 1, 12, 0, 2)),
          LocationUpdate(lat: 0.01, lng: 0.3, timestamp: DateTime(2026, 1, 1, 12, 0, 3)),
          LocationUpdate(lat: 0.01, lng: 0.4, timestamp: DateTime(2026, 1, 1, 12, 0, 4)),
          LocationUpdate(lat: 0.01, lng: 0.5, timestamp: DateTime(2026, 1, 1, 12, 0, 5)),
        ],
        interval: const Duration(milliseconds: 10),
      );

  group('NavigationNotifier lifecycle', () {
    test('start() moves idle -> starting -> navigating with a position fix', () async {
  ...
  group('NavigationNotifier rerouting', () {
    test('sustained deviation triggers a reroute from the current position', () async {
      final repo = _FakeNavRepository();
      ...
      expect(
        ...
        isTrue,
        reason: 'expected off-route detection then recovery, got ${state.status}',
      );
    ...
    test('failed reroute is retryable and never crashes navigation', () async {
      ...
      expect(state.status.isActive || state.status == NavigationStatus.error, isTrue);
    });
  });
  ...
  group('NavigationNotifier waypoints + destination', () {
    test('addStop records the waypoint and reroutes through it', () async {
      ...
      await notifier.addStop(const NavStop(label: 'Petrol pump', lat: 0, lng: 0.5));
