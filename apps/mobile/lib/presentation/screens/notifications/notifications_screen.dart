      ...
      body: SafeArea(
        child: isLoggedIn
            ? ListView(
                ...
                children: [
                  _PrefsCard(prefs: prefs, onChanged: (c, v) {
                    ref.read(notificationPrefsProvider.notifier).toggleCategory(c, v);
                  }),
                  const SizedBox(height: AppSpacing.lg),
                  ref.watch(notificationsProvider).when(
                        loading: () => const AppLoadingState(message: 'Loading notifications…'),
                        ...
                        data: (list) => _NotificationsList(
                          items: list,
                          onTap: (id) => ref.read(notificationsProvider.notifier).markRead(id),
                          onDelete: (id) => ref.read(notificationsProvider.notifier).delete(id),
                        ),
                      ),
                ],
              )
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: AppSpacing.lg),
            ElevatedButton(onPressed: onSignIn, child: const Text('Sign in to get notifications')),
          ],
        ),
      ),
    );
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Alert categories', style: Theme.of(context).textTheme.headlineSmall),
            ...
            SwitchListTile(
              ...
              subtitle: const Text('Alerts when a trip nears or passes its budget'),
              contentPadding: EdgeInsets.zero,
            ),
            SwitchListTile(
            ...
            SwitchListTile(
              ...
              subtitle: const Text('Optional marketing — off by default, and never required'),
              contentPadding: EdgeInsets.zero,
            ),
          ],
        ),
...
class _NotificationsList extends StatelessWidget {
  const _NotificationsList({required this.items, required this.onTap, required this.onDelete});
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty) {
      return const AppEmptyState(
        message: 'Nothing here yet — alerts will show up when there is trip news.',
    ...
    return Column(
      ...
      children: [
        ...
        ...items.map((n) => Card(
              ...
              child: ListTile(
                leading: Icon(_iconFor(n.type), color: n.read ? AppColors.textSecondary : AppColors.primary),
                title: Text(n.title ?? n.type, style: Theme.of(context).textTheme.bodyLarge),
                subtitle: n.body != null ? Text(n.body!, style: Theme.of(context).textTheme.bodySmall) : null,
                trailing: IconButton(
